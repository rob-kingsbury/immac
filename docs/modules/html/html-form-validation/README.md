...
That matches the letter-digit-letter, space, digit-letter-digit shape of a Canadian postal code, space optional. The `title` attribute is worth adding here: many browsers show its text in the validation tooltip when a value doesn't match, instead of a generic "please match the requested format" with no clue what the format actually is. That helps a sighted user in a browser that shows the tooltip, but it's not a substitute for a real accessibility fix. As covered in [Web Accessibility Fundamentals](/modules/accessibility/web-accessibility-fundamentals.md), `title` isn't announced consistently across screen readers, so a student who genuinely needs this format explained to everyone should also state the pattern in visible text near the field, not rely on `title` alone.
